...
public class SubtractController {
    ...
    private SubtractModel model;
    private CalculatorFrame view;

    public SubtractController(SubtractModel model, CalculatorFrame view) {
        this.model = model;
        this.view = view;
    }
        ...
        try {
            ...
            if (!view.getNumber1().isEmpty() && !view.getNumber2().isEmpty()) { //Verifica que no sean números vacios
                ...
                if (checkDecimalPlaces(field1) && checkDecimalPlaces(field2)) { //Verifia que no tenga mas de 3 numeros decimales
                    ...
                    double result = toThreeDecimals(model.subtract(num1, num2)); //Suma los numeros, luego formatea para maximo 3 digitos decimales y luego lo pasa a double
